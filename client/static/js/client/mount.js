...
function on_open(event) {
    ...
    document.getElementById("mountConnectBtn").addEventListener("click",mount_connect)
...
function ChangeConnectButtonStatus(){
    // Change button status when connectBtn is clicked
    is_mount_connected ? (
        document.getElementById("mountConnectBtn").classList.add("btn-success"),
        document.getElementById("mountConnectBtn").innerHTML='<i class="fas fa-link fa-spin mr-1"></i>已连接',
        SetBtnActive("#mountDisconnectBtn"),
        document.getElementById("mountDisconnectBtn").addEventListener("click", mount_disconnect)
    ):(
        document.getElementById("mountConnectBtn").classList.remove("btn-success"),		//如果已经连接
		document.getElementById("mountConnectBtn").innerHTML='<i class="fas fa-link mr-1"></i>连接',
		SetBtnActive("#mountConnectBtn"),
        SetBtnInactive("#mountDisconnectBtn")
    )
}

// ----------------------------------------------------------------
// Button Event Handlers
function mount_connect(){
    ...
    name = (name != undefined && name != null && name != "") ? name : "mount"
    type = (type != undefined) ? type : "ascom"

    console.debug("Trying to connect to mount ...")
    console.debug(host + ":" + port + ":" + name + ":" + type)
    log("Trying to connect to mount ...")
    let params = {
        host: host,
        port: port,
        name: name,
...
function mount_disconnect(){
    if(!is_mount_connected){
        console.log("Mount is not connected , please do not execute disconnect command")
    }
    SendRemoteDisconnect()
}

function mount_reconnect(){
    if(!is_mount_connected){
        console.log("Mount is not connected, please do not execute reconnect command")
    }
    SendRemoteReconnect()
}

function mount_scanning(){
    if(!is_mount_connected){
        console.log("Mount is not connected, please do not execute scanning command")
    }
    SendRemoteScanning()
}
