...
window.Win10 = {
        ...
        $('#win10').on('click', '.win10-open-window', function () {
            ...
            if ($this.data('url') !== "") {
                ...
                if ($this.data('icon-font')) {
                    //>> 加入到标题中
                    title = '<i class="fas fa-fw fa-' + $this.data('icon-font') + ' icon ' + bg + '"></i>' + title;
                }
    ...
    _init: function () {
        ...
        Win10.setContextMenu('#win10>.desktop', [
            ['<i class="fas fa-fw fa-star"></i> 收藏本页', function () {
            ...
            ['<i class="fas fa-fw fa-window-maximize"></i> ' + Win10.lang('进入全屏', 'Enable Full Screen'), function () { Win10.enableFullScreen() }],
            ['<i class="fas fa-fw fa-window-restore"></i> ' + Win10.lang('退出全屏', 'Disable Full Screen'), function () { Win10.disableFullScreen() }],
            '|',
            ['<i class="fas fa-fw fa-info-circle"></i> ' + Win10.lang('关于', 'About Us'), function () { Win10.aboutUs() }],
        ]);
        Win10.setContextMenu('#win10_btn_group_middle', [
            ['<i class="fas fa-fw fa-window-maximize"></i> ' + Win10.lang('全部显示', 'Show All Windows'), function () { Win10.showWins() }],
            ['<i class="fas fa-fw fa-window-minimize"></i> ' + Win10.lang('全部隐藏', 'Hide All Windows'), function () { Win10.hideWins() }],
            ['<i class="fas fa-fw fa-window-close"></i> ' + Win10.lang('全部关闭', 'Close All Windows'), function () { Win10.closeAll() }],
        ]);
    ...
    newMsg: function (title, content, handle_click) {
        var e = $('<div class="msg">' +
            ...
            '<span class="btn_close_msg fas fa-close"></span>' +
            '</div>');
    ...
    openUrl: function (url, title, areaAndOffset) {
        ...
        var btn = $('<div id="win10_' + index + '" index="' + index + '" class="btn show active"><div class="btn_title">' + title + '</div><div class="btn_close fas fa-close"></div></div>');
        var layero_opened = Win10.getLayeroByIndex(index);
        layero_opened.css('z-index', Win10._countTask + 813);
        Win10._settop(layero_opened);
        layero_opened.find('.layui-layer-setwin').prepend('<a class="win10-btn-change-url" index="' + index + '" title="' + Win10.lang('修改地址', 'Change URL') + '" href="javascript:void(0)"><span class="fas fa-chain"></span></a><a class="win10-btn-refresh" index="' + index + '" title="' + Win10.lang('刷新', 'Refresh') + '" href="javascript:void(0)"><span class="fas fa-refresh"></span></a>');
    ...
    aboutUs: function () {
        ...
        layer.open({
            ...
            title: 'LighAPT ' + this._version,
            shadeClose: true, //开启遮罩关闭
            area: ['320px', '200px'], //宽高
            content: '<div style="padding: 10px;font-size: 12px">' +
                '<p>LightAPT</p>' +
                '<p>Copyright @ Max Qian 2022</p>' +
                '<p>作者邮箱:qwdmax@qq.com</p>' +
                '</div>'
        });
    },
